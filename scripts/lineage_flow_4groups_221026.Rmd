...
dfPMSTissues <- mutate(dfPMSTissues, PMS = replace_na(dfPMSTissues$PMS, FALSE)) %>%
  complete(Disease_Status, PMS, fill = list(n = 0))
...
dfAllTissues <- mutate(dfAllTissues, all = replace_na(dfAllTissues$all, FALSE)) %>%
  complete(Disease_Status, all, fill = list(n = 0))
...
#Create a Tissue by Donor Grid
dfTissueGrid <- dfTissues %>%
  select(1:12) %>%
  select(!c(Thymus, Islet_Sup)) %>%
  relocate(PBMC, .after = Spleen) %>%
  relocate(HEAD_LN, .after = Disease_Status) %>%
  relocate(MES_LN, .after = Tail_LN) %>%
  mutate(Donor = gsub("HPAP", "", Donor))
dfTissueGrid <- dfTissueGrid %>%
  mutate(across(.cols = 3:ncol(dfTissueGrid), .fns = ~ ifelse(is.na(dfTissueGrid$.x), FALSE, TRUE)))
dfTissueGrid <- dfTissueGrid %>%
  pivot_longer(3:ncol(dfTissueGrid), names_to = "Tissue", values_to = "Analyzed")

tissueGrid <- ggplot(dfTissueGrid, aes(Donor, factor(Tissue, c("Whole_Blood", "PBMC", "Spleen", "SMA_LN", "MES_LN", "Tail_LN", "BODY_LN", "HEAD_LN")))) +
  geom_tile(aes(fill = Analyzed)) +
  scale_fill_brewer() +
  xlab("Donor ID") +
  theme_classic() +
...
Compare immune population frequencies between different scientists
```{r}
MetricPop <- unique(dfFresh$metric)
...
dfScientistStat <- dfFresh %>%
  ...
  group_by(metric) %>%
  wilcox_test(percent ~ Staining_Flow_Researcher) %>%
  adjust_pvalue() %>%
  filter(p.adj < 0.05)
...
g <- dfFresh %>%
  ...
  filter(Disease_Status == "ND") %>%
  ggplot(aes(Staining_Flow_Researcher, percent, color = Staining_Flow_Researcher)) +
  geom_jitter() +
  theme_classic() +
  facet_wrap(~metric)
