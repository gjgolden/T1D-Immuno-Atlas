...
dfCD8HeatmapMetadata <- dplyr::select(dfCD8Heatmap, Tissue)

dfCD8Heatmap <- dplyr::select(dfCD8Heatmap, "CD8 Tcm":ncol(dfCD8Heatmap))
...
dfCD4HeatmapMetadata <- dplyr::select(dfCD4Heatmap, Tissue)

dfCD4Heatmap <- dplyr::select(dfCD4Heatmap, "Tcm":ncol(dfCD4Heatmap))
...
BcellListStats <- dfBcellPopsStats %>%
  dplyr::select(B_cells_CD11cpos:ncol(dfBcellPopsStats)) %>%
  colnames()
...
CD4TcellListStats <- dfCD4TcellPopsStats %>%
  dplyr::select(CD4_Mem_gcTfh:ncol(dfCD4TcellPopsStats)) %>%
  colnames()
...
CD8TcellListStats <- dfCD8TcellPopsStats %>%
  dplyr::select(CD8_Mem_CD127negCD27neg:ncol(dfCD8TcellPopsStats)) %>%
  colnames()
...
NKcellListStats <- dfNKcellPopsStats %>%
  dplyr::select(NK_CD56briCD16neg:ncol(dfNKcellPopsStats)) %>%
  colnames()
...
#filtering out Scientist A's samples
dfCD4forPCA <- dfPCA %>%
  #filter(Tissue != "Spleen") %>%
  filter(!is.na(`CD4 Tcm CD25+`)) %>%
  dplyr::select(all_of(pcaCD4Pops))
...
CD4PCAplot <- dfCD4PCA %>%
  ...
  labs(x=paste0("PC3: ",round(CD4_var_explained[4]*100,1),"%"),
       y=paste0("PC4: ",round(CD4_var_explained[5]*100,1),"%")) +
  scale_color_manual(values = LinColorsOnly) +
  theme_classic()
...
```{r Section 6: Disease State - Checking cold ischemia differences}
...
coldIsRankedPlot
saveRDS(coldIsRankedPlot, file = "./outs/rds/cold-ischemia-ranked.RDS")
...
```{r Section 6: Disease State - Checking age differences}
...
#AgeFreqStats <- filter(dfAllPopsFreqStats, !is.na(Age)) %>%
  #filter(Disease_Status == "ND")
#AgePops <- colnames(coldIsFreqStats[15:ncol(coldIsFreqStats)])
...
#Tnaive cells negatively correlate with age, which is a known effect. The populations between the cold ischemia time and age do not overlap strongly, either.
```
...
CD25_CD38_boxplot_list <- list("CD25+_CD4_CD4+ T cells_pLN" , "CD25+_CD8_CD8+ T cells_pLN", "CD38+_CD4_CD4+ T cells_pLN", "CD38+_CD8_CD8+ T cells_pLN") #string will be split at _ for graph creation

CD25_CD38_facetLabels <- c("Tn", "Tnl", "Tcm", "Tem", "Temra")

lapply(CD25_CD38_boxplot_list, function(c) {
...
Section 7: Immune Population Frequency Across Number of Autoantibodies
```{r Section 7: Plotting certain }
TregsAAbs <- dfLineageFilter %>%
  filter(LN_type == "pLN") %>%
  filter(metric == "CD4 Mem Tregs") %>%
  ggplot(aes(Aabs, value, color = Aabs)) +
    geom_boxplot(width = 0.8,
                 outlier.shape = NA,
                 show.legend = FALSE) +
    geom_point(
