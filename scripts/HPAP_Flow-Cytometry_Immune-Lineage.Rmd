...
```{r Section 7: Disease State - CD25+ and CD38+ frequency across all T cell subsets}
...
dfDiseaseScales <- dfLineageFilter %>%
  ...
  group_by(metric, LN_type, `Disease Status`) %>%
  mutate(mean_z = mean(z_score)) %>%
  mutate(mean = mean(value))

CD25_disease_line_Zscore <- dfDiseaseScales %>%
    filter(grepl("CD25+", metric)) %>%
    filter(LN_type == "mLN") %>%
    ggplot(aes(`Disease Status`, mean_z, color = tpop, group = tpop)) +
    geom_point() +
    geom_line() +
    theme_classic() +
    theme(axis.title.x = element_blank(),
    ...
    facet_wrap(vars(cd, LN_type)) +
    theme(strip.text.x = element_blank())
CD25_disease_line_Zscore
ggsave(filename = "CD25_disease_line_Zscore.pdf",
       plot = CD25_disease_line_Zscore,
       device = "pdf",
       path = "./outs/pdf/",
       width = 3,
       height = 1.5,
...
CD25_disease_line_NonNorm <- dfDiseaseScales %>%
    filter(grepl("CD25+", metric)) %>%
    filter(LN_type == "pLN") %>%
    ggplot(aes(`Disease Status`, mean, color = tpop, group = tpop)) +
    geom_point() +
    geom_line() +
    theme_classic() +theme(axis.title.x = element_blank(),
          ...
          legend.title = element_text(size=8), #change legend title font size
          legend.text = element_text(size=6)) + #change legend text font size
    facet_wrap(vars(cd), scales = "free_y") +
    theme(strip.text.x = element_blank())
CD25_disease_line_NonNorm
ggsave(filename = "CD25_disease_line_NonNorm.pdf",
       plot = CD25_disease_line_NonNorm,
       device = "pdf",
       path = "./outs/pdf/",
       width = 3,
       height = 1.5,
...
CD38_disease_line_Zscore <- dfDiseaseScales %>%
    ...
    facet_wrap(vars(cd, LN_type)) +
    theme(strip.text.x = element_blank())
CD38_disease_line_Zscore
ggsave(filename = "CD38_disease_line_Zscore.pdf",
       plot = CD38_disease_line_Zscore,
       device = "pdf",
       path = "./outs/pdf/",
       width = 3,
       height = 1.5,
...
CD38_disease_line_NonNorm <- dfDiseaseScales %>%
    filter(grepl("CD38+", metric)) %>%
    filter(!grepl("HLA-DR+", metric)) %>%
    filter(LN_type == "pLN") %>%
    ggplot(aes(`Disease Status`, mean, color = tpop, group = tpop)) +
    ...
    facet_wrap(vars(cd)) +
    theme(strip.text.x = element_blank())
CD38_disease_line_NonNorm
ggsave(filename = "CD38_disease_line_NonNorm.pdf",
       plot = CD38_disease_line_NonNorm,
       device = "pdf",
       path = "./outs/pdf/",
       width = 3,
       height = 1.5,
...
CD25_CD38_boxplot_settings <- list(geom_boxplot(width = 0.8,
               outlier.shape = NA,
               show.legend = FALSE),
               geom_point(
                  size = 1,
...
CD25_boxplots_CD4 <- dfDiseaseScales %>%
  filter(grepl("CD25+", metric)) %>%
  filter(LN_type != "Spleen") %>%
  filter(cd == "CD4") %>%
  ggplot(aes(`Disease Status`, value, color = `Disease Status`)) +
    CD25_CD38_boxplot_settings +
    facet_grid(cols = vars(tpop), switch = "y") +
    theme(strip.text.x = element_blank())
CD25_boxplots_CD4
ggsave(filename = "CD25_boxplots_CD4.pdf",
       plot = CD25_boxplots_CD4,
       device = "pdf",
       path = "./outs/pdf/",
       width = 4,
       height = 1.5,
...
CD25_boxplots_CD8 <- dfDiseaseScales %>%
  filter(grepl("CD25+", metric)) %>%
  filter(LN_type != "Spleen") %>%
  filter(cd == "CD8") %>%
  ggplot(aes(`Disease Status`, value, color = `Disease Status`)) +
    ...
    theme(strip.text.x = element_blank())
CD25_boxplots_CD8
ggsave(filename = "CD25_boxplots_CD8.pdf",
       plot = CD25_boxplots_CD8,
       device = "pdf",
       path = "./outs/pdf/",
       width = 4,
       height = 1.5,
...
CD38_boxplots_CD4 <- dfDiseaseScales %>%
  filter(grepl("CD38+", metric)) %>%
  filter(!grepl("HLA-DR+", metric)) %>%
  filter(LN_type != "Spleen") %>%
  filter(cd == "CD4") %>%
  ggplot(aes(`Disease Status`, value, color = `Disease Status`)) +
    ...
    theme(strip.text.x = element_blank())
CD38_boxplots_CD4
ggsave(filename = "CD38_boxplots_CD4.pdf",
       plot = CD38_boxplots_CD4,
       device = "pdf",
       path = "./outs/pdf/",
       width = 4,
       height = 1.5,
...
CD38_boxplots_CD8 <- dfDiseaseScales %>%
  filter(grepl("CD38+", metric)) %>%
  filter(!grepl("HLA-DR+", metric)) %>%
  filter(LN_type != "Spleen") %>%
  filter(cd == "CD8") %>%
  ggplot(aes(`Disease Status`, value, color = `Disease Status`)) +
    ...
    theme(strip.text.x = element_blank())
CD38_boxplots_CD8
ggsave(filename = "CD38_boxplots_CD8.pdf",
       plot = CD38_boxplots_CD8,
       device = "pdf",
       path = "./outs/pdf/",
       width = 4,
       height = 1.5,
...
```{r}
diseaseColors <- c("#949494", "#2E96FF", "#FF8B3D")

testplot <- dfLineageFilter %>%
    filter(metric == "CD8 Tcm CD25+") %>%
    filter(LN_type == "pLN") %>%
    mutate(`Disease Status` = fct_relevel(`Disease Status`, "ND", "AAb+", "T1D")) %>%
    ggplot(aes(`Disease Status`, value, color = `Disease Status`)) +
    geom_boxplot(width = 0.8,
